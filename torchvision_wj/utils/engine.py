# using tensorboard implement saving output images in bothe training and validation,
# but for the scalers i want to put mutible models in the same figrue, i mean i want to show tow or more slops in the figure each with model title
        ...
        for batch_idx, (images, targets) in enumerate(
            ...
            if tb_logger:
                ...
                tb_logger.log_scalars(
                    batch_metrics, tb_logger.global_step, prefix="batch"
                )
...
# fix the ground_truth beacouze it shows only black image there is no picture to see try adding filter or grayscale
def log_validation_images(
    images, targets, seg_preds, epoch, batch_idx, tb_logger, is_test=False
):
    ...
    try:
        ...
        tb_logger.log_images(
            images_dict,
            epoch * 1000 + batch_idx,
            prefix="validation" if not is_test else "testing",
        )
    ...
    for batch_idx, (images, targets) in enumerate(
        ...
        if tb_logger and batch_idx % save_images_freq == 0:
            val_images = log_validation_images(
                images, targets, seg_preds, epoch, batch_idx, tb_logger, is_test
            )
